...
void CPatternGeneratorView::OnDraw(CDC* pDC)
{
	...
	CBrush brushyellow(RGB(250, 102, 0));
	CBrush* p01dBrush = pDC ->SelectObject(&brushyellow);//sun
	pDC->Ellipse(330, 230, 270, 170);
	pDC->SelectObject(p01dBrush);
	
	CBrush brushBlack1(RGB(0, 0, 0));
	CBrush* p02dBrush = pDC ->SelectObject(&brushBlack1); //mercury size
	pDC->Ellipse(x + 10, y + 10, x - 10, y - 10);
	pDC->SelectObject(p02dBrush);


	CBrush brushBlack2(RGB(0, 0, 0));
	CBrush* p03dBrush = pDC->SelectObject(&brushBlack2);//venus
	pDC->Ellipse(x1+10, y1+10, x1-10, y1-10);
	pDC->SelectObject(p03dBrush);

	CBrush brushBlack3(RGB(0, 0, 0));
	CBrush* p04dBrush = pDC->SelectObject(&brushBlack3);//earth
	pDC->Ellipse(x2 + 10, y2 + 10, x2 - 10, y2 - 10);
	pDC->SelectObject(p04dBrush);
